...
**Current status:** Live production has passed the previous bot-protection/build-fix cycle. Access control is locked down across Payload collections. Checkout supports saved-address selection and stamps last-used dates. Address Book management is built. R2 digital delivery, license-scaled allowance, Promotions, Gifting, Sequenzy email helpers, and email-link auth are built. SEO/AI layer and security headers are shipped. Public API bot protection is in place for contact, newsletter signup, and privacy request endpoints. Shared transactional email branding has been updated to the three-line site-style header: `Benny & Penny` / `♥ Adventures ♥` / `MEDICAL BOOKS FOR BRAVE LITTLE HEARTS`. Admin System Status Check includes Sequenzy above Mailjet. Customer portal mobile layout has been patched for session navigation, Orders, and Library. Admin mobile sidebar and search placement are now much improved.

**Current active concern:** The customer portal is substantially improved but needs real-device validation. The admin dashboard mobile experience remains the active blocker, but the problem has narrowed: sidebar/hamburger is now reported as working well, search is now below the greeting, and the remaining issue is mobile dashboard row/table layout. The highlighted System Status, Recent Orders, and Community Growth data rows were being forced into stacked one-column mobile rows by dashboard CSS. A final stylesheet, `app/(payload)/admin-dashboard-mobile-rows.scss`, was created and imported last in `app/(payload)/layout.tsx` to restore compact table-like rows inside mobile cards. This latest row-layout fix needs deployment/phone validation before new features.
...
**Current next actions:** (1) Verify latest `admin-dashboard-mobile-rows.scss` deployment and phone screenshots. (2) Confirm System Status rows, Recent Orders rows, and Community Growth rows use the available right-side space and no longer stack unnecessarily. (3) Confirm sidebar/hamburger remains stable after row override. (4) Validate customer portal mobile pages: Portal Home, Orders, Library, Gifts, and Addresses. (5) Confirm compact System Status logos are showing correctly. (6) Confirm whether the Sequenzy footer badge can be removed through account settings. (7) Continue the email failover plan with Sequenzy as primary transactional email and Mailjet as fallback. See `[C] Backlog & Launch Checklist.md`, `[C] Admin Dashboard and Portal Mobile Progress 2026-06-16.md`, and `[C] 2026-06-16.md` for details.
