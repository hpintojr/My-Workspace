...
02 Projects/MCD CRM - Agent and Admin Portals/[C] GHL Production Build-Out Runbook.md  (exact GHL workflows, tag registry, AI-workflow-builder prompts, phased checklist — read before touching GHL)
...
02 Projects/MCD CRM - Agent and Admin Portals/[C] v1.2 Business-Terms Reconciliation & GHL Flows.md  (reconciles scope to approved business terms + the two agent-GHL flows + GHL e-sign onboarding)
02 Projects/MCD CRM - Agent and Admin Portals/[C] Automated Agent Onboarding Flow (GHL-first).md  (signup → GHL e-sign → MiniCRM provisioning)
02 Projects/MCD CRM - Agent and Admin Portals/[C] Automated Onboarding Sequence.svg
02 Projects/MCD CRM - Agent and Admin Portals/[C] AI Handoff & Scope Review.md  (READ FIRST for any AI on the CRM — doc map, build state, coding conventions, gap analysis)
02 Projects/MCD CRM - Agent and Admin Portals/[C] Build Specs — Index & Roadmap.md  (slice-by-slice specs 01–15; full specs 01–04 written, 05–15 outlined; mirrored in repo docs/)
```

Approved business terms locked (v1.2, 2026-06-30 — full reconciliation in [C] v1.2 Business-Terms Reconciliation & GHL Flows.md):

```txt
Legal entity: Charter Oaks Assets, Inc. d/b/a Mercury Call Desk; signer Hamilton Pinto Jr.; 231 E Alessandro Blvd A-208, Riverside CA; emails hello@/support@/sales@mercurycalldesk.com; interim caller ID (909) 276-7631.
Commission MODEL CHANGE: 50/50 of Net Commissionable Profit (collected retail − published partner wholesale − actual Stripe fee); sales tax excluded. This REPLACES the old tiered 40/35/30 gross-profit model. Wholesale schedule: Std Starter $1,595/$1,000, Std Growth $1,995/$1,250, Std Pro $3,995/$1,500, Ent Starter $5,295/$2,500, Ent Growth $7,595/$3,500, Ent Pro $9,995/$5,000. Engine needs processing_fee on the funding payload + wholesale lookup. Setup-fee commission: after first-month wholesale+fee, remainder 50/50 (min setup-profit Starter $1k/Growth $1.5k/Pro $3k/Ent $5k).
Payout SLAs: new-account first payment + setup within 30 days after cleared funds AND a launch checklist complete; established/renewals within 15 days; scope/price/structure change = new contract. Refund/chargeback = hold/offset future commissions, 10-day repay window, 30-day written dispute, CRM notes are controlling evidence.
Lead rules: cold lead protected ONLY after documented two-way contact; referrals protected on accurate entry; OpenPool releases a cold lead with no demo after 45 days; Shark Tank (stalled contract-priced) reassignable only to top-tier closers/managers; credit earned only on close + cleared payment; no standard MCD splits; MCD may move any account to House.
Service/residual: MANDATORY cadence per package (Starter biweekly→monthly, Growth weekly→biweekly/monthly, Pro/Ent weekly + guaranteed 1-hr weekly) — this REPLACES the old scope §12.3 'healthy accounts need no activity'. 60-day no-health-confirm = violation (1st warning, 2nd in rolling 12 mo = account to House); 3-day no client response+escalate may justify termination. Support intake support@mercurycalldesk.com; MCD L1 intake + L2 escalation; 24h target.
Onboarding: mandatory gate + Hamilton COUNTERSIGNATURE before access. E-sign via GHL Documents & Contracts (multi-recipient = countersignature; legal audit cert with signer IP/timestamps); GHL 'signed/completed' workflow trigger → webhook → MiniCRM flips the gate. Retain signed PDF + agreement version + signer IP/audit trail + countersignature + W-9 (completion state only, never raw tax/bank data) + profile completion + certification. 7-year retention. This supersedes the earlier R2-only e-sign assumption for the agreements (still mirror signed PDF to R2 for the 7-yr record).
Agent GHL presence (BOTH being piloted to compare): Flow A = dedicated GHL backend user per partner in a 'Sales HQ' sub-account with Only Assigned Data but NO agent login issued (MiniCRM acts via the token; user exists for native assignment/round-robin); Flow B = no per-agent GHL user, attribution via Contact custom fields (mini_crm_agent_id, originating_agent_id) + a small service-user pool. Recommended hybrid: A as assigned owner + B's custom fields for MiniCRM lineage. Either way agents NEVER log into GHL; other clients (separate sub-accounts) + true/wholesale pricing stay hidden.
DNC: total blackout (calls, SMS, sales email, marketing email, social DMs); transactional/billing/security/outage may continue; log any opt-out within 24h. Customer email only via MCD CRM/company email; no partner SMS program yet.
Automated onboarding (GHL-first, 2026-06-30): Hamilton's only manual steps = create the GHL user + IONOS mailbox + a confirm call + one 'agent-approved' tag; the rest automates. Public signup at crm.mercurycalldesk.com/signup ports data to a GHL contact; the tag fires a GHL workflow that e-signs contract/NDA/W-9 — documents LIVE IN GHL (Documents & Contracts), MiniCRM keeps only refs + completion state (offload). GHL 'completed' webhook provisions the MiniCRM user + emails an activation link to the agent's PERSONAL email; agent then finishes profile + banking in-portal. SSN entered ONLY inside the W-9 e-sign doc (never a stored/queryable field); banking via a secure payout provider (token/completion only, no raw account numbers); IONOS default password should force a change on first login (don't keep a shared standing password). MiniCRM stays the leads/commission/compliance engine + agent portal feeding opportunities to GHL. Details: [C] Automated Agent Onboarding Flow (GHL-first).md.
Attorney review of the agreements is still pending before signature; Advanced-Partner minimum-margin floor still to be set.
```

Build kickoff (repo hpintojr/crm.mcd, local D:\GitHub\crm.mcd) — 2026-06-30: Next.js 15 App Router (src/) + TypeScript strict + Tailwind (dark) + Prisma + Postgres (Neon) + Vercel. Present: config, prisma schema (User/Agent/OnboardingDocument/Certification/AuditLog + enums), src/lib (db singleton, server-only env, stub-safe GHL client, zod validation), landing page, public /signup form, and POST /api/signup (validate → GHL Sales HQ upsert → create SUBMITTED agent + 4 onboarding-doc gates → audit). Runs before GHL is connected (GHL client returns a stub id). ChatGPT does most coding; Claude reviews/cleans. The full build is specced slice-by-slice in [C] Build Specs — Index & Roadmap.md (specs 01–04 full, 05–15 outlined; mirrored in repo docs/ as BUILD-SPEC-*.md so ChatGPT works down the list and Claude reviews each against its acceptance criteria). Read [C] AI Handoff & Scope Review.md FIRST — it holds the doc map, coding conventions, and the prioritized gap analysis (P1: auth stack, rate limiting, webhook shared-secret model since GHL Custom Webhooks don't HMAC-sign, background-worker host, national DNC/TCPA scrub, payout+1099 onboarding). Code guardrails: no SSN/tax-ID field (W-9 e-sign only, retrievable from the signed PDF), no raw bank data (payout token only), EMAIL_ACCESS_TOKEN server-only (agents never log into email), GHL one-way + agents never log into GHL, audit every sensitive action. P1 build decisions locked (2026-06-30): Auth = Auth.js (NextAuth v5, argon2id, TOTP MFA); public-route protection = Cloudflare WAF Free + a rate-limit rule on /api/signup (crm.mercurycalldesk.com must be Cloudflare-proxied in front of Vercel) + honeypot; GHL API via the provided Sales HQ Location ID + token, inbound webhooks authed by a shared-secret header + location-id allowlist + idempotency (GHL Custom Webhooks do NOT HMAC-sign); scraping/enrichment = a LOCAL Python script writing leads into Neon (no cloud worker host in v1), nurture email via Sequenzy (Mailjet fallback) needing a defined campaign/workflow + suppression sync; calling = manual B2B business-line dialing ONLY (no dialer, no cell/personal numbers) — lowers TCPA/DNC exposure but keep internal DNC/opt-out logging + a business-phone field; payouts = Stripe Connect (Express) via a hosted onboarding link, CRM stores connected-account id + payouts_enabled only (Stripe handles 1099).

**Open problems (updated 2026-07-01):** GHL Private Integration Token is confirmed working (MCP connected against the live "Mercury Call Desk" location, `lEdLVFW0uqKMhmkgFrsX`) — the token/access item is DONE. What's actually open is building the GHL side: no MCD-specific custom fields, tags, or workflows exist in that location yet. Full spec (exact workflows, tag registry, AI-workflow-builder prompts, phased Build/1.5/2 checklist) is in `[C] GHL Production Build-Out Runbook.md` — Phase 1 (the onboarding e-sign workflow, trigger = tag `agent-approved`) is the only thing blocking the end-to-end applicant test. `mcd@gmail.com` demo calendar/Meet setup is still pending (separate from the IONOS mailbox below). Onboarding/training: the CRM consumes the existing agent-onboarding library in 02 Projects/MCD - Mercury Call Desk/01-agent-onboarding/ as the single source of truth — Sales Partner Agreement/NDA-IP/W-9/Acknowledgment are registration gates (R2 e-sign; store W-9 completion state only), the Certification Scorecard flips a can_claim_leads flag (ACTIVE != allowed to claim), the Manager New Hire Checklist becomes the admin activation screen, and Welcome/Product/ICP/Scripts/CRM SOP/Compliance/Demo + collateral become the NDA-gated knowledge base + training modules. Do NOT fork those files; CRM ingests/publishes versions. Map lives in [C] Agent Registration, Knowledge Base & Training Integration.md.

**Activation email (fixed 2026-07-01):** Code review of Slice 03 found activation links were generated but never actually delivered to agents (only echoed into the GHL webhook's HTTP response, which nothing consumed). Fixed with real SMTP send (`src/lib/mail.ts`, nodemailer) through the new `no-reply@mercurycalldesk.com` IONOS mailbox — Hamilton created the mailbox and set `EMAIL_SMTP_USER`/`EMAIL_SMTP_PASSWORD`/etc. in Vercel (using the same token as `EMAIL_ACCESS_TOKEN`). Also fixed a concurrency race where two simultaneous document-completion webhooks could both try to create the same agent's user row. The old GHL-tag-based activation approach (`invite-delivery.ts` / `triggerPortalInvite()`) is confirmed dead code — safe to delete on the next cleanup pass, do not build a GHL workflow for its `mcd-portal-invite` tag.
