...
```txt
Phase 0 result: NO Salesforce→GHL automation exists. The runbook's §2.3 third branch applies:
  "the automation does not exist and must be built. The calendar change alone will NOT fix this."

Evidence (proven live 2026-08-13 evening, E2E test lead FCA E2E TwoWay 20260813T3):
  - SF lead 00QV500000fr7WoMAI created via live funnel; GHL contact MiCyWvjbgkOAxIb5HhBg,
    appointment PARQ5xNTWpoHKDvhMUWh (Aug 20 2:00 PM PT), opportunity jpS8aIYSgPcl42LlnmYj.
  - Lead Owner changed Web Leads → Kenny Jumps in SF at 2026-08-13 20:34:10 PT.
  - Flow "F&C Lead Owner to Salesforce Calendar" (301V500000oWTDNIA4, V1, ACTIVE, Hamilton
    8/12/2026 9:54 PM PT) fired: record-triggered on Lead update, condition
    ISCHANGED({!$Record.OwnerId}); single element "Move F&C Events to New Lead Owner"
    (Update Records, Salesforce Events only). SF Event correctly moved to Kenny Jumps.
  - The flow contains NO callout to GHL, no HTTP action, no external service. Nothing was
    sent toward GHL, so nothing could be rejected.
  - GHL verified via MCP at +1 min and +2.5 min after owner change: contact assignedTo,
    opportunity assignedTo, appointment assignedUserId ALL still Alex (8tTyPhJCYmCqsCFvaiq6).
  - Reinterpretation of the 20260813T2 SyncTest: since no SF→GHL automation exists, the
    contact/opportunity flip to Kenny at 23:16Z must have been performed GHL-side (where the
    personal calendar then refused only the appointment leg). The Overview's "SF workflow is
    firing and being rejected" hypothesis is DISPROVEN for the SF-side path.
  - Steps 0.3/0.4 (agent email): flow "Lead Assignment Notification" (301V500000aR07OIAS, V6,
    Sergey 2/10/2026) exists but is INACTIVE. No agent notification email fires on assignment
    today. If reactivated it must pass the no-GHL-links constraint check first.
  - Step 0.5 (sync-failure log at 23:16Z): not yet read — log location still an open question.
  - Internal lead notification email (scope §47, GHL workflow → hpintojr@gmail.com +
    josh@advantagefirst.com): NOT received. Hamilton checked hpintojr@gmail.com at ~9:42 PM PT;
    nothing near the 8:31 PM lead creation. A full-mailbox search for "Free & Clear Advantage
    Lead" (incl. spam) returned nothing — CONFIRMED ABSENT. Combined with the empty GHL custom
    fields: the GHL-side config (custom-field mapping + internal email workflow) was never
    built. Both are new build items, not regressions.
  - Also noted: an orange "Check status before owner change" banner appeared in SF on owner
    change; "Owner Changed" audit Tasks are generated by a separate automation
    ("Generated by: System Administrator (API)"), which DID fire for our change.

Decided by: Claude (E2E test authorized by Hamilton)   Date: 2026-08-13
```

**Consequence for scope:** the cutover now has TWO required work items, not one:
(1) the team calendar replacement (Phases 1–3, unchanged), and
(2) BUILD the Salesforce→GHL assignment sync — e.g. extend the F&C flow with an outbound
    action to the Vercel app or GHL API that writes contact.assignedTo, opportunity.assignedTo,
    and appointment.assignedUserId on the new agent. Re-scope with Hamilton before building
    (lock: proposal only). Additionally, GHL custom fields (debt amount/types, employment,
    payment status, consent set) arrived EMPTY on the E2E contact — the scope §31–32 mapping
    is not populating and needs its own fix.
