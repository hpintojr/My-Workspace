...
Lead pool, lifecycle, ownership, acquisition source, campaign, engagement channel, and website opportunity are separate concepts. Do not overwrite one to represent another.

Example:

```txt
A business independently identified through Google Maps research
...
|---|---|---|
| `GOOGLE_MAPS` | Google Maps | Prospect was independently identified through manual Google Maps / Google Business Profile research. Do not use this label to authorize automated extraction or storage of Google Maps content. |
...
sourceRecordUrl          original profile, ad/landing page, form URL, or similar evidence where allowed
...
A Google Maps-researched prospect who later replies to email is recorded as:

```txt
originalSource = GOOGLE_MAPS
intakeMethod = MANUAL_ENTRY or approved API import
...
|---|---|---|
| **Cold Pool / Prospects** | Fresh validated business prospects not yet in nurture and without documented two-way contact. | Company-owned records. Controlled import, assignment, DNC/suppression, and activity logging are required. |
...
| **Open Pool** | Booked-demo no-shows plus records eligible for governed release. | Qualifying agents may claim under the controlled first-come process. Release reason must be retained. |
...
## 5. Website incentive and website-only lane

A missing website must be treated as an opportunity, not an automatic conclusion.
...
→ verified no website
→ offer review
→ MCD package with approved website incentive OR website-only quote
```

Website fields:

```txt
websiteStatus             UNKNOWN | LISTED | NO_WEBSITE_LISTED | VERIFIED_NO_WEBSITE | NEEDS_REVIEW
websiteReviewedAt         review timestamp
websiteReviewNote         evidence/assessment summary
proposalType              MCD_PACKAGE | WEBSITE_ONLY | MCD_WITH_INCLUDED_WEBSITE
proposalStatus            DRAFT | SENT | ACTIVE | ACCEPTED | DECLINED | EXPIRED | CANCELLED | SUPERSEDED
quotedAmount              website-only quote amount where applicable
quoteExpiresAt            active quote window
```

A website-only quote is currently expected to remain within the approved $500–$3,000 range. It must carry a manager-approved scope, price, and expiry before an agent represents it to the prospect.

## 6. Lifecycle and pool movement examples
...
No listed website → Verified no website → Bundle incentive or Website-only quote
...
- Source, campaign, pool, owner, lifecycle, every claim, reassignment, release, no-show, proposal, quote-expiry, website review, and suppression event must be auditable.
...
- Do not activate live assignment, pool claims, marketing automation, proposal delivery, or commission logic until schema migration and controlled test cases are approved.
...
- intake method
- current pool and governed pool-movement history
- source correction audit history
- website-status review and proposal/quote foundation
- source, pool, owner, lifecycle, campaign, and website-status filters in Admin
- source/pool/website opportunity visibility in Agent lead cards without exposing confidential provider credentials or unapproved pricing
