...
- [ ] 🟠 **Rotate `PAYLOAD_SETUP_SECRET`** in Vercel (was deferred — now due; it was exposed). (2.2)
...
- [ ] 🟡 **Bot/spam protection** on `/api/contact`, `/api/subscribe`, `/api/privacy-request`: honeypot + Turnstile + per-IP rate limit. (2.4)
...
- [ ] 🟢 **Gifting Phase 3 — marketing automation:** tag + nurture gifted recipients (needs Mailjet); attribution reporting.

- [ ] 🟡 **Sequenzy email automations:** receipt + set-password link, gift-redeemed confirmation, password reset, dunning, review request. (Provider wired; gift-delivery email shipped.)
